...
사용자가 직접 선택합니다. 한 sample을 여러 Reference와 비교하는 것도 가능합니다.
표준 ONT run 폴더에 `fastq_pass`가 있으면 그 안의 passing FASTQ만 분석하며,
`fastq_fail`, `other_reports`, `pod5_*`는 자동으로 제외합니다.
